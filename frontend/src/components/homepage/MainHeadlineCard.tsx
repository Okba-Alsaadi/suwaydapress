...
export default function MainHeadlineCard({ article, locale, dict, showVideoBadge }: Props) {
  // Main image: use the ORIGINAL for maximum sharpness
  const imageUrl = article.featured_image?.url;
  // Blurred background: use the tiny thumbnail (fast, no performance hit)
  const blurredImageUrl =
    article.featured_image?.formats?.thumbnail?.url || article.featured_image?.url;

  const fullUrl = imageUrl ? `${process.env.NEXT_PUBLIC_STRAPI_URL}${imageUrl}` : null;
  const blurredUrl = blurredImageUrl
    ? `${process.env.NEXT_PUBLIC_STRAPI_URL}${blurredImageUrl}`
    : null;
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-[16/9] bg-gray-200">
        {/* Blurred background – tiny thumbnail, blurs to fill */}
        {blurredUrl && (
        ...
        {/* Main image – original resolution, never cropped */}
        {fullUrl && (
